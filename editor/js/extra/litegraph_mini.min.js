var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(x,m,t){x!=Array.prototype&&x!=Object.prototype&&(x[m]=t.value)};$jscomp.getGlobal=function(x){return"undefined"!=typeof window&&window===x?x:"undefined"!=typeof global&&null!=global?global:x};$jscomp.global=$jscomp.getGlobal(this);
...
0;b<this.list_of_graphcanvas.length;++b)c=this.list_of_graphcanvas[b],c.selected_nodes[a.id]&&delete c.selected_nodes[a.id],c.node_dragged==a&&(c.node_dragged=null);b=this._nodes.indexOf(a);-1!=b&&this._nodes.splice(b,1);delete this._nodes_by_id[a.id];if(this.onNodeRemoved)this.onNodeRemoved(a);this.sendActionToCanvas("checkPanels");this.setDirtyCanvas(!0,!0);this.afterChange();this.change();this.updateExecutionOrder()}};m.prototype.getNodeById=function(a){return null==a?null:this._nodes_by_id[a]};
m.prototype.findNodesByClass=function(a,b){b=b||[];for(var c=b.length=0,d=this._nodes.length;c<d;++c)this._nodes[c].constructor===a&&b.push(this._nodes[c]);return b};m.prototype.findNodesByType=function(a,b){a=a.toLowerCase();b=b||[];for(var c=b.length=0,d=this._nodes.length;c<d;++c)this._nodes[c].type.toLowerCase()==a&&b.push(this._nodes[c]);return b};m.prototype.findNodeByTitle=function(a){for(var b=0,c=this._nodes.length;b<c;++b)if(this._nodes[b].title==a)return this._nodes[b];return null};m.prototype.findNodesByTitle=
function(a){for(var b=[],c=0,d=this._nodes.length;c<d;++c)this._nodes[c].title==a&&b.push(this._nodes[c]);return b};m.prototype.getNodeOnPos=function(a,b,c,d){c=c||this._nodes;for(var e=c.length-1;0<=e;e--){var g=c[e];if(g.isPointInside(a,b,d))return g}return null};m.prototype.getGroupOnPos=function(a,b){for(var c=this._groups.length-1;0<=c;c--){var d=this._groups[c];if(d.isPointInside(a,b,2,!0))return d}return null};m.prototype.checkNodeTypes=function(){for(var a=0;a<this._nodes.length;a++){var b=
this._nodes[a];if(b.constructor!=f.registered_node_types[b.type]){console.log("node being replaced by newer version: "+b.type);var c=f.createNode(b.type);this._nodes[a]=c;c.configure(b.serialize());c.graph=this;this._nodes_by_id[c.id]=c;b.inputs&&(c.inputs=b.inputs.concat());b.outputs&&(c.outputs=b.outputs.concat())}}this.updateExecutionOrder()};m.prototype.onAction=function(a,b,c){this._input_nodes=this.findNodesByClass(f.GraphInput,this._input_nodes);for(var d=0;d<this._input_nodes.length;++d){var e=
this._input_nodes[d];if(e.properties.name==a){e.actionDo(a,b,c);break}}};m.prototype.trigger=function(a,b){if(this.onTrigger)this.onTrigger(a,b)};m.prototype.addInput=function(a,b,c){if(!this.inputs[a]){this.beforeChange();this.inputs[a]={name:a,type:b,value:c};this._version++;this.afterChange();if(this.onInputAdded)this.onInputAdded(a,b);if(this.onInputsOutputsChange)this.onInputsOutputsChange()}};m.prototype.setInputData=function(a,b){if(a=this.inputs[a])a.value=b};m.prototype.getInputData=function(a){return(a=
this.inputs[a])?a.value:null};m.prototype.renameInput=function(a,b){if(b!=a){if(!this.inputs[a])return!1;if(this.inputs[b])return console.error("there is already one input with that name"),!1;this.inputs[b]=this.inputs[a];delete this.inputs[a];this._version++;if(this.onInputRenamed)this.onInputRenamed(a,b);if(this.onInputsOutputsChange)this.onInputsOutputsChange()}};m.prototype.changeInputType=function(a,b){if(!this.inputs[a])return!1;if(!this.inputs[a].type||String(this.inputs[a].type).toLowerCase()!=
String(b).toLowerCase())if(this.inputs[a].type=b,this._version++,this.onInputTypeChanged)this.onInputTypeChanged(a,b)};m.prototype.removeInput=function(a){if(!this.inputs[a])return!1;delete this.inputs[a];this._version++;if(this.onInputRemoved)this.onInputRemoved(a);if(this.onInputsOutputsChange)this.onInputsOutputsChange();return!0};m.prototype.addOutput=function(a,b,c){this.outputs[a]={name:a,type:b,value:c};this._version++;if(this.onOutputAdded)this.onOutputAdded(a,b);if(this.onInputsOutputsChange)this.onInputsOutputsChange()};
m.prototype.setOutputData=function(a,b){if(a=this.outputs[a])a.value=b};m.prototype.getOutputData=function(a){return(a=this.outputs[a])?a.value:null};m.prototype.renameOutput=function(a,b){if(!this.outputs[a])return!1;if(this.outputs[b])return console.error("there is already one output with that name"),!1;this.outputs[b]=this.outputs[a];delete this.outputs[a];this._version++;if(this.onOutputRenamed)this.onOutputRenamed(a,b);if(this.onInputsOutputsChange)this.onInputsOutputsChange()};m.prototype.changeOutputType=
...
m.prototype.setDirtyCanvas=function(a,b){this.sendActionToCanvas("setDirty",[a,b])};m.prototype.removeLink=function(a){if(a=this.links[a]){var b=this.getNodeById(a.target_id);b&&b.disconnectInput(a.target_slot)}};m.prototype.serialize=function(){for(var a=[],b=0,c=this._nodes.length;b<c;++b)a.push(this._nodes[b].serialize());c=[];for(b in this.links){var d=this.links[b];if(!d.serialize){console.warn("weird LLink bug, link info is not a LLink but a regular object");var e=new t;for(g in d)e[g]=d[g];
d=this.links[b]=e}c.push(d.serialize())}var g=[];for(b=0;b<this._groups.length;++b)g.push(this._groups[b].serialize());a={last_node_id:this.last_node_id,last_link_id:this.last_link_id,nodes:a,links:c,groups:g,config:this.config,extra:this.extra,version:f.VERSION};if(this.onSerialize)this.onSerialize(a);return a};m.prototype.configure=function(a,b){if(a){b||this.clear();b=a.nodes;if(a.links&&a.links.constructor===Array){for(var c=[],d=0;d<a.links.length;++d){var e=a.links[d];if(e){var g=new t;g.configure(e);
c[g.id]=g}else console.warn("serialized graph link data contains errors, skipping.")}a.links=c}for(d in a)"nodes"!=d&&"groups"!=d&&(this[d]=a[d]);c=!1;this._nodes=[];if(b){d=0;for(e=b.length;d<e;++d){g=b[d];var h=f.createNode(g.type,g.title);h||(f.debug&&console.log("Node not found or has errors: "+g.type),h=new l,h.last_serialization=g,c=h.has_errors=!0);h.id=g.id;this.add(h,!0)}d=0;for(e=b.length;d<e;++d)g=b[d],(h=this.getNodeById(g.id))&&h.configure(g)}this._groups.length=0;if(a.groups)for(d=0;d<
a.groups.length;++d)b=new f.LGraphGroup,b.configure(a.groups[d]),this.add(b);this.updateExecutionOrder();this.extra=a.extra||{};if(this.onConfigure)this.onConfigure(a);this._version++;this.setDirtyCanvas(!0,!0);return c}};m.prototype.load=function(a,b){var c=this;if(a.constructor===File||a.constructor===Blob){var d=new FileReader;d.addEventListener("load",function(a){a=JSON.parse(a.target.result);c.configure(a);b&&b()});d.readAsText(a)}else{var e=new XMLHttpRequest;e.open("GET",a,!0);e.send(null);
e.onload=function(a){200!==e.status?console.error("Error loading graph:",e.status,e.response):(a=JSON.parse(e.response),c.configure(a),b&&b())};e.onerror=function(a){console.error("Error loading graph:",a)}}};m.prototype.onNodeTrace=function(a,b,c){};t.prototype.configure=function(a){a.constructor===Array?(this.id=a[0],this.origin_id=a[1],this.origin_slot=a[2],this.target_id=a[3],this.target_slot=a[4],this.type=a[5]):(this.id=a.id,this.type=a.type,this.origin_id=a.origin_id,this.origin_slot=a.origin_slot,
this.target_id=a.target_id,this.target_slot=a.target_slot)};t.prototype.serialize=function(){return[this.id,this.origin_id,this.origin_slot,this.target_id,this.target_slot,this.type]};f.LLink=t;x.LGraphNode=f.LGraphNode=l;l.prototype._ctor=function(a){this.title=a||"Unnamed";this.size=[f.NODE_WIDTH,60];this.graph=null;this._pos=new Float32Array(10,10);Object.defineProperty(this,"pos",{set:function(a){!a||2>a.length||(this._pos[0]=a[0],this._pos[1]=a[1])},get:function(){return this._pos},enumerable:!0});
...
(g.mode===f.ON_TRIGGER?(d.action_call||(d.action_call=this.id+"_trigg_"+Math.floor(9999*Math.random())),g.onExecute&&g.doExecute(b,d)):g.onAction&&(d.action_call||(d.action_call=this.id+"_act_"+Math.floor(9999*Math.random())),h=g.inputs[h.target_slot],g.actionDo(h.name,b,d)))}}}};l.prototype.clearTriggeredSlot=function(a,b){if(this.outputs&&(a=this.outputs[a])&&(a=a.links)&&a.length)for(var c=0;c<a.length;++c){var d=a[c];if(null==b||b==d)if(d=this.graph.links[a[c]])d._last_time=0}};l.prototype.setSize=
...
f.auto_load_slot_types&&f.registerNodeAndSlotType(this,b,!0);this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0);return a};l.prototype.addOutputs=function(a){for(var b=0;b<a.length;++b){var c=a[b],d={name:c[0],type:c[1],link:null};if(a[2])for(var e in c[2])d[e]=c[2][e];this.outputs||(this.outputs=[]);this.outputs.push(d);if(this.onOutputAdded)this.onOutputAdded(d);f.auto_load_slot_types&&f.registerNodeAndSlotType(this,c[1],!0)}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeOutput=
...
function(a,b){var c=new Float32Array(2);if(this.inputs)for(var d=0,e=this.inputs.length;d<e;++d){var g=this.inputs[d];this.getConnectionPos(!0,d,c);if(D(a,b,c[0]-10,c[1]-5,20,10))return{input:g,slot:d,link_pos:c}}if(this.outputs)for(d=0,e=this.outputs.length;d<e;++d)if(g=this.outputs[d],this.getConnectionPos(!1,d,c),D(a,b,c[0]-10,c[1]-5,20,10))return{output:g,slot:d,link_pos:c};return null};l.prototype.findInputSlot=function(a,b){if(!this.inputs)return-1;for(var c=0,d=this.inputs.length;c<d;++c)if(a==
...
if(!b)return!1;var c=this.inputs[a].link;if(null!=c){this.inputs[a].link=null;var d=this.graph.links[c];if(d){var e=this.graph.getNodeById(d.origin_id);if(!e)return!1;var g=e.outputs[d.origin_slot];if(!g||!g.links||0==g.links.length)return!1;for(var h=0,n=g.links.length;h<n;h++)if(g.links[h]==c){g.links.splice(h,1);break}delete this.graph.links[c];this.graph&&this.graph._version++;if(this.onConnectionsChange)this.onConnectionsChange(f.INPUT,a,!1,d,b);if(e.onConnectionsChange)e.onConnectionsChange(f.OUTPUT,
...
c){this._pos[0]+=a;this._pos[1]+=b;if(!c)for(c=0;c<this._nodes.length;++c){var d=this._nodes[c];d.pos[0]+=a;d.pos[1]+=b}};w.prototype.recomputeInsideNodes=function(){this._nodes.length=0;for(var a=this.graph._nodes,b=new Float32Array(4),c=0;c<a.length;++c){var d=a[c];d.getBounding(b);B(this._bounding,b)&&this._nodes.push(d)}};w.prototype.isPointInside=l.prototype.isPointInside;w.prototype.setDirtyCanvas=l.prototype.setDirtyCanvas;f.DragAndScale=A;A.prototype.bindEvents=function(a){this.last_mouse=
new Float32Array(2);this._binded_mouse_callback=this.onMouse.bind(this);f.pointerListenerAdd(a,"down",this._binded_mouse_callback);f.pointerListenerAdd(a,"move",this._binded_mouse_callback);f.pointerListenerAdd(a,"up",this._binded_mouse_callback);a.addEventListener("mousewheel",this._binded_mouse_callback,!1);a.addEventListener("wheel",this._binded_mouse_callback,!1)};A.prototype.computeVisibleArea=function(a){if(this.element){var b=this.element.width,c=this.element.height,d=-this.offset[0],e=-this.offset[1];
a&&(d+=a[0]/this.scale,e+=a[1]/this.scale,b=a[2],c=a[3]);a=d+b/this.scale;c=e+c/this.scale;this.visible_area[0]=d;this.visible_area[1]=e;this.visible_area[2]=a-d;this.visible_area[3]=c-e}else this.visible_area[0]=this.visible_area[1]=this.visible_area[2]=this.visible_area[3]=0};A.prototype.onMouse=function(a){if(this.enabled){var b=this.element,c=b.getBoundingClientRect(),d=a.clientX-c.left;c=a.clientY-c.top;a.canvasx=d;a.canvasy=c;a.dragging=this.dragging;var e=!this.viewport||this.viewport&&d>=
...
return b};A.prototype.mouseDrag=function(a,b){this.offset[0]+=a/this.scale;this.offset[1]+=b/this.scale;if(this.onredraw)this.onredraw(this)};A.prototype.changeScale=function(a,b){a<this.min_scale?a=this.min_scale:a>this.max_scale&&(a=this.max_scale);if(a!=this.scale&&this.element){var c=this.element.getBoundingClientRect();if(c&&(b=b||[.5*c.width,.5*c.height],c=this.convertCanvasToOffset(b),this.scale=a,.01>Math.abs(this.scale-1)&&(this.scale=1),a=this.convertCanvasToOffset(b),a=[a[0]-c[0],a[1]-
c[1]],this.offset[0]+=a[0],this.offset[1]+=a[1],this.onredraw))this.onredraw(this)}};A.prototype.changeDeltaScale=function(a,b){this.changeScale(this.scale*a,b)};A.prototype.reset=function(){this.scale=1;this.offset[0]=0;this.offset[1]=0};x.LGraphCanvas=f.LGraphCanvas=k;k.DEFAULT_BACKGROUND_IMAGE="data:image/png;base64,";
...
if(this.onDrawOverlay)this.onDrawOverlay(a);c&&a.restore();a.finish2D&&a.finish2D()}};k.prototype.drawSubgraphPanel=function(a){var b=this.graph,c=b._subgraph_node;c?(this.drawSubgraphPanelLeft(b,c,a),this.drawSubgraphPanelRight(b,c,a)):console.warn("subgraph without subnode")};k.prototype.drawSubgraphPanelLeft=function(a,b,c){var d=b.inputs?b.inputs.length:0,e=Math.floor(1.6*f.NODE_SLOT_HEIGHT);c.fillStyle="#111";c.globalAlpha=.8;c.beginPath();c.roundRect(10,10,200,(d+1)*e+50,[8]);c.fill();c.globalAlpha=
...
!0};var H=new Float32Array(2);k.prototype.drawNode=function(a,b){this.current_node=a;var c=a.color||a.constructor.color||f.NODE_DEFAULT_COLOR,d=a.bgcolor||a.constructor.bgcolor||f.NODE_DEFAULT_BGCOLOR,e=.6>this.ds.scale;if(this.live_mode){if(!a.flags.collapsed&&(b.shadowColor="transparent",a.onDrawForeground))a.onDrawForeground(b,this,this.canvas)}else{var g=this.editor_alpha;b.globalAlpha=g;this.render_shadows&&!e?(b.shadowColor=f.DEFAULT_SHADOW_COLOR,b.shadowOffsetX=2*this.ds.scale,b.shadowOffsetY=
2*this.ds.scale,b.shadowBlur=3*this.ds.scale):b.shadowColor="transparent";if(!a.flags.collapsed||!a.onDrawCollapsed||1!=a.onDrawCollapsed(b,this)){var h=a._shape||f.BOX_SHAPE;H.set(a.size);var n=a.horizontal;if(a.flags.collapsed){b.font=this.inner_text_font;var p=a.getTitle?a.getTitle():a.title;null!=p&&(a._collapsed_width=Math.min(a.size[0],b.measureText(p).width+2*f.NODE_TITLE_HEIGHT),H[0]=a._collapsed_width,H[1]=0)}a.clip_area&&(b.save(),b.beginPath(),h==f.BOX_SHAPE?b.rect(0,0,H[0],H[1]):h==f.ROUND_SHAPE?
b.roundRect(0,0,H[0],H[1],[10]):h==f.CIRCLE_SHAPE&&b.arc(.5*H[0],.5*H[1],.5*H[0],0,2*Math.PI),b.clip());a.has_errors&&(d="red");this.drawNodeShape(a,b,H,c,d,a.is_selected,a.mouseOver);b.shadowColor="transparent";if(a.onDrawForeground)a.onDrawForeground(b,this,this.canvas);b.textAlign=n?"center":"left";b.font=this.inner_text_font;d=!e;var r=this.connecting_output;h=this.connecting_input;b.lineWidth=1;p=0;var u=new Float32Array(2);if(!a.flags.collapsed){if(a.inputs)for(c=0;c<a.inputs.length;c++){var q=
a.inputs[c],y=q.type,k=q.shape;b.globalAlpha=g;this.connecting_output&&!f.isValidConnection(q.type,r.type)&&(b.globalAlpha=.4*g);b.fillStyle=null!=q.link?q.color_on||this.default_connection_color_byType[y]||this.default_connection_color.input_on:q.color_off||this.default_connection_color_byTypeOff[y]||this.default_connection_color_byType[y]||this.default_connection_color.input_off;var v=a.getConnectionPos(!0,c,u);v[0]-=a.pos[0];v[1]-=a.pos[1];p<v[1]+.5*f.NODE_SLOT_HEIGHT&&(p=v[1]+.5*f.NODE_SLOT_HEIGHT);
b.beginPath();"array"==y&&(k=f.GRID_SHAPE);q.type===f.EVENT||q.shape===f.BOX_SHAPE?n?b.rect(v[0]-5+.5,v[1]-8+.5,10,14):b.rect(v[0]-6+.5,v[1]-5+.5,14,10):k===f.ARROW_SHAPE?(b.moveTo(v[0]+8,v[1]+.5),b.lineTo(v[0]-4,v[1]+6+.5),b.lineTo(v[0]-4,v[1]-6+.5),b.closePath()):k===f.GRID_SHAPE?(b.rect(v[0]-4,v[1]-4,2,2),b.rect(v[0]-1,v[1]-4,2,2),b.rect(v[0]+2,v[1]-4,2,2),b.rect(v[0]-4,v[1]-1,2,2),b.rect(v[0]-1,v[1]-1,2,2),b.rect(v[0]+2,v[1]-1,2,2),b.rect(v[0]-4,v[1]+2,2,2),b.rect(v[0]-1,v[1]+2,2,2),b.rect(v[0]+
2,v[1]+2,2,2)):e?b.rect(v[0]-4,v[1]-4,8,8):b.arc(v[0],v[1],4,0,2*Math.PI);b.fill();d&&(y=null!=q.label?q.label:q.name)&&(b.fillStyle=f.NODE_TEXT_COLOR,n||q.dir==f.UP?b.fillText(y,v[0],v[1]-10):b.fillText(y,v[0]+10,v[1]+5))}b.textAlign=n?"center":"right";b.strokeStyle="black";if(a.outputs)for(c=0;c<a.outputs.length;c++)if(q=a.outputs[c],y=q.type,k=q.shape,this.connecting_input&&!f.isValidConnection(y,h.type)&&(b.globalAlpha=.4*g),v=a.getConnectionPos(!1,c,u),v[0]-=a.pos[0],v[1]-=a.pos[1],p<v[1]+.5*
f.NODE_SLOT_HEIGHT&&(p=v[1]+.5*f.NODE_SLOT_HEIGHT),b.fillStyle=q.links&&q.links.length?q.color_on||this.default_connection_color_byType[y]||this.default_connection_color.output_on:q.color_off||this.default_connection_color_byTypeOff[y]||this.default_connection_color_byType[y]||this.default_connection_color.output_off,b.beginPath(),"array"==y&&(k=f.GRID_SHAPE),r=!0,y===f.EVENT||k===f.BOX_SHAPE?n?b.rect(v[0]-5+.5,v[1]-8+.5,10,14):b.rect(v[0]-6+.5,v[1]-5+.5,14,10):k===f.ARROW_SHAPE?(b.moveTo(v[0]+8,
v[1]+.5),b.lineTo(v[0]-4,v[1]+6+.5),b.lineTo(v[0]-4,v[1]-6+.5),b.closePath()):k===f.GRID_SHAPE?(b.rect(v[0]-4,v[1]-4,2,2),b.rect(v[0]-1,v[1]-4,2,2),b.rect(v[0]+2,v[1]-4,2,2),b.rect(v[0]-4,v[1]-1,2,2),b.rect(v[0]-1,v[1]-1,2,2),b.rect(v[0]+2,v[1]-1,2,2),b.rect(v[0]-4,v[1]+2,2,2),b.rect(v[0]-1,v[1]+2,2,2),b.rect(v[0]+2,v[1]+2,2,2),r=!1):e?b.rect(v[0]-4,v[1]-4,8,8):b.arc(v[0],v[1],4,0,2*Math.PI),b.fill(),!e&&r&&b.stroke(),d&&(y=null!=q.label?q.label:q.name))b.fillStyle=f.NODE_TEXT_COLOR,n||q.dir==f.DOWN?
...
-h),b.fill());if(a.onDrawTitle)a.onDrawTitle(b)}if(g){if(a.onBounding)a.onBounding(C);r==f.TRANSPARENT_TITLE&&(C[1]-=e,C[3]+=e);b.lineWidth=1;b.globalAlpha=.8;b.beginPath();p==f.BOX_SHAPE?b.rect(-6+C[0],-6+C[1],12+C[2],12+C[3]):p==f.ROUND_SHAPE||p==f.CARD_SHAPE&&a.flags.collapsed?b.roundRect(-6+C[0],-6+C[1],12+C[2],12+C[3],[2*this.round_radius]):p==f.CARD_SHAPE?b.roundRect(-6+C[0],-6+C[1],12+C[2],12+C[3],[2*this.round_radius,2,2*this.round_radius,2]):p==f.CIRCLE_SHAPE&&b.arc(.5*c[0],.5*c[1],.5*c[0]+
6,0,2*Math.PI);b.strokeStyle=f.NODE_BOX_OUTLINE_COLOR;b.stroke();b.strokeStyle=d;b.globalAlpha=1}0<a.execute_triggered&&a.execute_triggered--;0<a.action_triggered&&a.action_triggered--};var L=new Float32Array(4),G=new Float32Array(4),K=new Float32Array(2),J=new Float32Array(2);k.prototype.drawConnections=function(a){var b=f.getTime(),c=this.visible_area;L[0]=c[0]-20;L[1]=c[1]-20;L[2]=c[2]+40;L[3]=c[3]+40;a.lineWidth=this.connections_width;a.fillStyle="#AAA";a.strokeStyle="#AAA";a.globalAlpha=this.editor_alpha;
c=this.graph._nodes;for(var d=0,e=c.length;d<e;++d){var g=c[d];if(g.inputs&&g.inputs.length)for(var h=0;h<g.inputs.length;++h){var n=g.inputs[h];if(n&&null!=n.link&&(n=this.graph.links[n.link])){var p=this.graph.getNodeById(n.origin_id);if(null!=p){var r=n.origin_slot;var u=-1==r?[p.pos[0]+10,p.pos[1]+10]:p.getConnectionPos(!1,r,K);var q=g.getConnectionPos(!0,h,J);G[0]=u[0];G[1]=u[1];G[2]=q[0]-u[0];G[3]=q[1]-u[1];0>G[2]&&(G[0]+=G[2],G[2]=Math.abs(G[2]));0>G[3]&&(G[1]+=G[3],G[3]=Math.abs(G[3]));if(B(G,
L)){var y=p.outputs[r];r=g.inputs[h];if(y&&r&&(p=y.dir||(p.horizontal?f.DOWN:f.RIGHT),r=r.dir||(g.horizontal?f.UP:f.LEFT),this.renderLink(a,u,q,n,!1,0,null,p,r),n&&n._last_time&&1E3>b-n._last_time)){y=2-.002*(b-n._last_time);var k=a.globalAlpha;a.globalAlpha=k*y;this.renderLink(a,u,q,n,!0,y,"white",p,r);a.globalAlpha=k}}}}}}a.globalAlpha=1};k.prototype.renderLink=function(a,b,c,d,e,g,h,n,p,r){d&&this.visible_links.push(d);!h&&d&&(h=d.color||k.link_type_colors[d.type]);h||(h=this.default_link_color);
null!=d&&this.highlighted_links[d.id]&&(h="#FFF");n=n||f.RIGHT;p=p||f.LEFT;var u=E(b,c);this.render_connections_border&&.6<this.ds.scale&&(a.lineWidth=this.connections_width+4);a.lineJoin="round";r=r||1;1<r&&(a.lineWidth=.5);a.beginPath();for(var q=0;q<r;q+=1){var y=5*(q-.5*(r-1));if(this.links_render_mode==f.SPLINE_LINK){a.moveTo(b[0],b[1]+y);var l=0,v=0,m=0,w=0;switch(n){case f.LEFT:l=-.25*u;break;case f.RIGHT:l=.25*u;break;case f.UP:v=-.25*u;break;case f.DOWN:v=.25*u}switch(p){case f.LEFT:m=-.25*
...
if(e.onNodeOutputAdd)e.onNodeOutputAdd(a.value);e.setDirtyCanvas(!0,!0);e.graph.afterChange()}else{a=[];for(var r in c)a.push({content:r,value:c[r]});new f.ContextMenu(a,{event:b,callback:g,parentMenu:d,node:e});return!1}}if(e){var h=this;a=k.active_canvas.getCanvasWindow();b=e.optional_outputs;e.onGetOutputs&&(b=e.onGetOutputs());var n=[];if(b)for(var p=0;p<b.length;p++){var r=b[p];if(!r)n.push(null);else if(!e.flags||!e.flags.skip_repeated_outputs||-1==e.findOutputSlot(r[0])){var u=r[0];r[2]||(r[2]=
{});r[2].label&&(u=r[2].label);r[2].removable=!0;u={content:u,value:r};r[1]==f.EVENT&&(u.className="event");n.push(u)}}this.onMenuNodeOutputs&&(n=this.onMenuNodeOutputs(n));f.do_add_triggers_slots&&-1==e.findOutputSlot("onExecuted")&&n.push({content:"On Executed",value:["onExecuted",f.EVENT,{nameLocked:!0}],className:"event"});e.onMenuNodeOutputs&&(b=e.onMenuNodeOutputs(n))&&(n=b);if(n.length)return new f.ContextMenu(n,{event:c,callback:g,parentMenu:d,node:e},a),!1}};k.onShowMenuNodeProperties=function(a,
b,c,d,e){if(e&&e.properties){var g=k.active_canvas;b=g.getCanvasWindow();var h=[],n;for(n in e.properties){a=void 0!==e.properties[n]?e.properties[n]:" ";"object"==typeof a&&(a=JSON.stringify(a));var p=e.getPropertyInfo(n);if("enum"==p.type||"combo"==p.type)a=k.getPropertyPrintableValue(a,p.values);a=k.decodeHTML(a);h.push({content:"<span class='property_name'>"+(p.label?p.label:n)+"</span><span class='property_value'>"+a+"</span>",value:n})}if(h.length)return new f.ContextMenu(h,{event:c,callback:function(a,
...
e.setDirtyCanvas(!0,!0)}};k.prototype.showLinkMenu=function(a,b){var c=this,d=c.graph.getNodeById(a.origin_id),e=c.graph.getNodeById(a.target_id),g=!1;d&&d.outputs&&d.outputs[a.origin_slot]&&(g=d.outputs[a.origin_slot].type);var h=!1;e&&e.outputs&&e.outputs[a.target_slot]&&(h=e.inputs[a.target_slot].type);var n=new f.ContextMenu(["Add Node",null,"Delete",null],{event:b,title:null!=a.data?a.data.constructor.name:null,callback:function(b,r,f){switch(b){case "Add Node":k.onMenuAdd(null,null,f,n,function(b){b.inputs&&
...
nodeNewType)}}return!1};k.prototype.showConnectionMenu=function(a){a=a||{};var b=Object.assign({nodeFrom:null,slotFrom:null,nodeTo:null,slotTo:null,e:null},a),c=this,d=b.nodeFrom&&b.slotFrom;a=!d&&b.nodeTo&&b.slotTo;if(!d&&!a)return console.warn("No data passed to showConnectionMenu"),!1;a=d?b.nodeFrom:b.nodeTo;var e=d?b.slotFrom:b.slotTo,g=!1;switch(typeof e){case "string":g=d?a.findOutputSlot(e,!1):a.findInputSlot(e,!1);e=d?a.outputs[e]:a.inputs[e];break;case "object":g=d?a.findOutputSlot(e.name):
a.findInputSlot(e.name);break;case "number":g=e;e=d?a.outputs[e]:a.inputs[e];break;default:return console.warn("Cant get slot information "+e),!1}a=["Add Node",null];c.allow_searchbox&&(a.push("Search"),a.push(null));var h=e.type==f.EVENT?"_event_":e.type,n=d?f.slot_types_default_out:f.slot_types_default_in;if(n&&n[h])if("object"==typeof n[h]||"array"==typeof n[h])for(var p in n[h])a.push(n[h][p]);else a.push(n[h]);var r=new f.ContextMenu(a,{event:b.e,title:(e&&""!=e.name?e.name+(h?" | ":""):"")+
(e&&h?h:""),callback:function(a,f,n){switch(a){case "Add Node":k.onMenuAdd(null,null,n,r,function(a){d?b.nodeFrom.connectByType(g,a,h):b.nodeTo.connectByTypeOutput(g,a,h)});break;case "Search":d?c.showSearchBox(n,{node_from:b.nodeFrom,slot_from:e,type_filter_in:h}):c.showSearchBox(n,{node_to:b.nodeTo,slot_from:e,type_filter_out:h});break;default:c.createDefaultNodeForSlot(Object.assign(b,{position:[b.e.canvasX,b.e.canvasY],nodeType:a}))}}});return!1};k.onShowPropertyEditor=function(a,b,c,d,e){function g(){if(p){var b=
p.value;"Number"==a.type?b=Number(b):"Boolean"==a.type&&(b=!!b);e[h]=b;n.parentNode&&n.parentNode.removeChild(n);e.setDirtyCanvas(!0,!0)}}var h=a.property||"title";b=e[h];var n=document.createElement("div");n.is_modified=!1;n.className="graphdialog";n.innerHTML="<span class='name'></span><input autofocus type='text' class='value'/><button>OK</button>";n.close=function(){n.parentNode&&n.parentNode.removeChild(n)};n.querySelector(".name").innerText=h;var p=n.querySelector(".value");p&&(p.value=b,p.addEventListener("blur",
...
2+1,p.NODE_TITLE_HEIGHT,[0,0,8,8]):a.roundRect(this.size[0]/2,b,this.size[0]/2+1,p.NODE_TITLE_HEIGHT,[0,0,8,8]),c?a.fill():a.fillRect(0,b,this.size[0]+1,p.NODE_TITLE_HEIGHT),a.textAlign="center",a.font="24px Arial",a.fillStyle=c?"#DDD":"#999",a.fillText("+",.25*this.size[0],b+24),a.fillText("+",.75*this.size[0],b+24))};t.prototype.onMouseDown=function(a,b,c){a=this.size[1]-p.NODE_TITLE_HEIGHT+.5;console.log(0);b[1]>a&&(b[0]<this.size[0]/2?(console.log(1),c.showSubgraphPropertiesDialog(this)):(console.log(2),
...
(function(x){function m(){this.addInput("","string");this.addOutput("table","table");this.addOutput("rows","number");this.addProperty("value","");this.addProperty("separator",",");this._table=null}x=x.LiteGraph;x.wrapFunctionAsNode("string/toString",function(m){if(m&&m.constructor===Object)try{return JSON.stringify(m)}catch(l){}return String(m)},[""],"string");x.wrapFunctionAsNode("string/compare",function(m,l){return m==l},["string","string"],"boolean");x.wrapFunctionAsNode("string/concatenate",
function(m,l){return void 0===m?l:void 0===l?m:m+l},["string","string"],"string");x.wrapFunctionAsNode("string/contains",function(m,l){return void 0===m||void 0===l?!1:-1!=m.indexOf(l)},["string","string"],"boolean");x.wrapFunctionAsNode("string/toUpperCase",function(m){return null!=m&&m.constructor===String?m.toUpperCase():m},["string"],"string");x.wrapFunctionAsNode("string/split",function(m,l){null==l&&(l=this.properties.separator);if(null==m)return[];if(m.constructor===String)return m.split(l||
